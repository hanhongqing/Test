...
def parse_args():
    ...
    parser.add_argument('--path_data',default = os.path.join("..","Images")) # path of input images
...
def get_image_filenames(path0):
    img_l = glob.glob(os.path.join(path0,"*.tif"))
...
def prepare_dataset(args):
    ...
    for i in range(len(img_l)):
        I[0,i] = tifffile.imread(os.path.join(args.path_data,img_l[i]+"-DNA.tif"))
        I[1,i] = tifffile.imread(os.path.join(args.path_data,img_l[i]+"-pH3.tif"))
        I[2,i] = tifffile.imread(os.path.join(args.path_data,img_l[i]+"-actin.tif"))
